...
class Command(BaseCommand):
    help = 'Populate product classifications by inferring from product names'

    def handle(self, *args, **options):
        # Process SellerProduct classifications (marketplace products posted by sellers)
        # NOTE: OPAS/CSV products are now in OPASProduct table - use populate_opas_classifications instead
        ...
        self.stdout.write(self.style.SUCCESS(f"SELLER PRODUCT CLASSIFICATION POPULATOR"))
        self.stdout.write(self.style.SUCCESS(f"(Marketplace Products Only)"))
        ...
        self.stdout.write(f"Searching for SellerProducts with empty classifications...")
        self.stdout.write(f"Found {total} products to classify...\n")
        self.stdout.write(f"Note: For OPAS/CSV products, use: manage.py populate_opas_classifications\n")
